...
- Variables that should be *constant* across all files should be written **directly into the template text** (replace `${var}` with the literal value), not left as empty columns — empty values produce warnings and, if a description field ends up empty, a bad upload.
