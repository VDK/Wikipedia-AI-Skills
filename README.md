...
| [flickr](.claude/skills/flickr/SKILL.md) | Fetch Flickr photos via the read-only API (photosets, search, metadata) and prepare pattypan upload manifests for Wikimedia Commons batch uploads - license filtering, attribution, and date handling (incl. the date_taken-as-upload-date pitfall and download rate-limit handling) |
| [pattypan](.claude/skills/pattypan/SKILL.md) | Build valid pattypan upload spreadsheets (.xls) for batch Wikimedia Commons uploads - two-sheet format, template variables, filename validation, and the bundled generator script |
| | **▸ Search & Data** |
